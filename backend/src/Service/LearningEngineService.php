...
class LearningEngineService
{
    ...
    public function getPatterns(array $criteria = []): array
    {
        if (isset($criteria['agent_type'])) {
            $patterns = $this->patternRepo->findByPatternType($criteria['agent_type']);
    ...
    private function extractPatternsFromInteraction(AgentInteraction $interaction): void
    {
        ...
        if ($existingPattern) {
            ...
            $this->createNewPattern($interaction, $contextSignature);
        }
    }
    ...
    private function calculateRecencyBonus(?\DateTimeInterface $lastUse): float
    {
        ...
        if ($daysSinceUse <= 30) return 0.03;
        if ($daysSinceUse <= 90) return 0.01;
    ...
    private function getPatternUsageHistory(KnowledgePattern $pattern): array
    {
        ...
        $interactions = $this->interactionRepo->findByPatternHash((string) $patternId);
